...
class main_window(QtWidgets.QMainWindow, Prototype):
    ...
    def __init__(self):
        ...
        #instrucciones
        self.instru = Instrucciones()
        self.instrucc = self.findChild(QtWidgets.QPushButton, 'instrucciones')
        self.instrucc.clicked.connect(self.abrirInstru)


        #bugs
        self.bug = Bugs()
        self.bugs = self.findChild(QtWidgets.QPushButton, 'bugs')
        self.bugs.clicked.connect(self.abrirBugs)

        #info
        self.info = Info()
        self.info1 = self.findChild(QtWidgets.QPushButton, 'info')
        self.info1.clicked.connect(self.abrirInfo)

        #app
        self.app = App()
        self.app1 = self.findChild(QtWidgets.QPushButton, 'app')
        self.app1.clicked.connect(self.abrirApp)
    ...
    def abrirInstru(self, eve):
        self.instru.exec_()

    def abrirBugs(self, eve):
        self.bug.exec_()

    def abrirInfo(self, eve):
        self.info.exec_()

    def abrirApp(self, eve):
        self.app.exec_()

class Instrucciones(QDialog):
    def __init__(self):
        QDialog.__init__(self)
        uic.loadUi("instrucciones.ui",self)
        self.setMinimumSize(800,600)
        self.setMaximumSize(800,600)
        self.setWindowTitle('¿Cómo funciona?') 

class Bugs(QDialog):
    def __init__(self):
        QDialog.__init__(self)
        uic.loadUi("bugs.ui",self)
        self.setMinimumSize(300,200)
        self.setMaximumSize(300,200)
        self.setWindowTitle('Errores') 

class Info(QDialog):
    def __init__(self):
        QDialog.__init__(self)
        uic.loadUi("info.ui",self)
        self.setMinimumSize(890,701)
        self.setMaximumSize(890,701)
